...
complete -c pkill -s e -l echo --description 'Display name and PID of the process being killed. RB ( pkill only. ).'
...
complete -c pkill -s O -l older --description 'Select processes older than secs.'
...
complete -c pkill -s w -l lightweight --description 'Shows all thread ids instead of pids in  pgrep \'s or  pidwait \'s context.'
complete -c pkill -s x -l exact --description 'Only match processes whose names (or command lines if -f is specified)  exact…'
complete -c pkill -s F -l pidfile --description 'Read PIDs from file.'
...
complete -c pkill -s q -l queue --description 'Use  sigqueue(3) rather than  kill(2) and the value argument is used to speci…'
